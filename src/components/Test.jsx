...
import { ShoppingBag, Search, Menu, ArrowRight, ChevronRight, X, Star } from 'lucide-react';
...
const Test = () => {
  ...
  // Product Data
  const products = [
    { id: 1, title: "Velvet Crescent", price: "$3,200", img: "https://images.unsplash.com/photo-1555041469-a586c61ea9bc?auto=format&fit=crop&q=80&w=800" },
    { id: 2, title: "Geometric Prism", price: "$1,850", img: "https://images.unsplash.com/photo-1524758631624-e2822e304c36?auto=format&fit=crop&q=80&w=800" },
    { id: 3, title: "Orbital Pendant", price: "$950", img: "https://images.unsplash.com/photo-1513506003901-1e6a229e2d15?auto=format&fit=crop&q=80&w=800" },
    { id: 4, title: "Minimalist Arc", price: "$2,400", img: "https://images.unsplash.com/photo-1583847268964-b28dc8f51f92?auto=format&fit=crop&q=80&w=800" },
    { id: 5, title: "Modular Haven", price: "$5,600", img: "https://images.unsplash.com/photo-1493663284031-b733aefaa830?auto=format&fit=crop&q=80&w=800" },
    { id: 6, title: "Linear Flow", price: "$1,200", img: "https://images.unsplash.com/photo-1586023492125-27b2c045efd7?auto=format&fit=crop&q=80&w=800" },
  ];
  ...
  return (
    <div className="min-h-screen bg-[#FDFBF7] text-stone-900 font-sans selection:bg-stone-300 overflow-x-hidden">
      
      <nav className="fixed w-full z-50 px-8 py-6 flex justify-between items-center bg-[#FDFBF7]/80 backdrop-blur-md transition-all duration-300">
        <div className="text-2xl font-bold tracking-tighter uppercase cursor-pointer hover:opacity-70 transition-opacity">Smart Furniture</div>
        <div className="hidden md:flex gap-8 text-xs uppercase tracking-[0.2em] font-medium">
          {['Collections', 'Designers', 'Lighting', 'Journal'].map(item => (
            <a key={item} href="#" className="hover:text-stone-500 transition-colors">{item}</a>
          ))}
        </div>
        <div className="flex gap-6">
          <Search size={20} className="cursor-pointer hover:scale-110 transition-transform" />
          <ShoppingBag size={20} className="cursor-pointer hover:scale-110 transition-transform" />
          <Menu size={20} className="md:hidden cursor-pointer" onClick={() => setIsMenuOpen(!isMenuOpen)} />
        </div>
      </nav>
      ...
      <section className="relative h-[90vh] flex items-center overflow-hidden">
        <div className="absolute inset-0 z-0">
           <div className="absolute inset-0 bg-stone-200 animate-pulse delay-700" />
           <img src="https://images.unsplash.com/photo-1618221195710-dd6b41faaea6?auto=format&fit=crop&q=80&w=2000" className="w-full h-full object-cover opacity-80" alt="Hero" />
        </div>
        <div className="relative z-10 px-12 md:px-24">
          <h1 className="text-7xl md:text-9xl font-light leading-[0.9] mb-8 animate-fade-in-up">
            Defined<br />by Design.
          </h1>
          <button className="flex items-center gap-3 bg-stone-900 text-white px-10 py-5 hover:bg-stone-700 transition-all uppercase text-xs tracking-[0.2em] group">
            Discover Collection <ArrowRight size={16} className="group-hover:translate-x-2 transition-transform" />
          </button>
        </div>
      </section>
      ...
      <section className="py-32 px-8 md:px-24">
        <div className="flex justify-between items-end mb-20 scroll-reveal">
          <div>
            <h2 className="text-5xl font-light uppercase tracking-tight mb-4">Curated Selections</h2>
            <p className="text-stone-500 max-w-sm">Discover pieces that define the modern landscape, curated for the bold and the curious.</p>
          </div>
          <span className="hidden md:flex text-stone-500 uppercase text-xs tracking-widest items-center gap-2 cursor-pointer hover:text-black">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {products.map((product) => (
            <div key={product.id} className="group cursor-pointer scroll-reveal">
              <div className="aspect-[3/4] bg-stone-200 overflow-hidden mb-6 relative">
                <img 
                  src={product.img} 
                  alt={product.title}
                ...
                <button className="absolute bottom-6 left-6 bg-white px-6 py-3 opacity-0 translate-y-4 group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-300 text-xs uppercase tracking-widest">
                  Quick View
                </button>
              </div>
      ...
      {/* Newsletter */}
      <section className="py-32 bg-stone-900 text-white px-8 md:px-24 flex flex-col items-center text-center">
        <h2 className="text-4xl md:text-6xl font-light mb-8">Join the Movement</h2>
        <p className="text-stone-400 mb-12 max-w-lg">Receive curated design insights and exclusive access to our private collection.</p>
        <div className="flex flex-col md:flex-row gap-4 w-full max-w-xl">
          <input 
            type="email" 
            placeholder="Enter your email" 
            className="flex-1 bg-transparent border border-stone-700 px-6 py-4 outline-none focus:border-white transition-colors"
          />
          <button className="bg-white text-stone-900 px-8 py-4 hover:bg-stone-200 transition-colors uppercase text-xs tracking-[0.2em] font-bold">
            Subscribe
          </button>
        </div>
      </section>

      <footer className="py-20 px-8 md:px-24 bg-[#FDFBF7]">
        <div className="text-center text-stone-400 text-xs uppercase tracking-widest">
          © 2026 Smart Furniture. All rights reserved. Designed for the curious.
        </div>
      </footer>
